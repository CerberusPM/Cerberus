...
class SecondPositionSubcommand extends BaseSubCommand {
    ...
    public function onRun(CommandSender $sender, string $alias, array $args): void {
        if (isset($args["position"])) { //Set the second position to specified position
            ...
            SelectionManager::selectSecondPosition($sender, Position::fromObject($args["position"], $world));
            $sender->sendMessage($this->config_manager->getPrefix() . $this->lang_manager->translate("command.pos2.selected.world", [$args["position"]->getFloorX(),
            ...
            if ($sender instanceof Player) {
                ...
                SelectionManager::selectSecondPosition($sender, $position);
                $sender->sendMessage($this->config_manager->getPrefix() . $this->lang_manager->translate("command.pos2.selected", [$position->getFloorX(),
                            $position->getFloorY(),
                            $position->getFloorZ()]));
            } else {
