...
class SecondPositionSubcommand extends CerberusSubcommand {
    ...
    public function onRun(CommandSender $sender, string $alias, array $args): void {
        if (isset($args["position"])) { //Set the second position to specified position
            ...
            $sender->sendMessage($this->lang_manager->translate("command.pos2.selected.world", [$args["position"]->getFloorX(),
            ...
            if ($sender instanceof Player) {
                ...
                $sender->sendMessage($this->lang_manager->translate("command.pos2.selected", [$position->getFloorX(),
                            $position->getFloorY(),
                            $position->getFloorZ()]));
            } else {
